"""Rust translator for DevCovenant LanguageUnit generation."""
...
TEST_TEMPLATES = ("{stem}_test.rs", "tests/{stem}.rs")
SYMBOL_PATTERNS = (
    ("function", re.compile(r"\bfn\s+([A-Za-z_][A-Za-z0-9_]*)")),
    ("struct", re.compile(r"\bstruct\s+([A-Za-z_][A-Za-z0-9_]*)")),
    ("variable", re.compile(r"\blet\s+([A-Za-z_][A-Za-z0-9_]*)")),
)
RISK_PATTERNS = (
    (re.compile(r"\bunsafe\b"), "Avoid unsafe blocks without justification."),
    (re.compile(r"\bstd::process::Command\b"), "Review process execution."),
)
...
def _has_nearby_comment(line_number: int, lines: list[str]) -> bool:
    """Return True when a nearby Rust comment marker is present."""
...
def translate(
    *, path: Path, source: str, declaration: TranslatorDeclaration, **_: Any
    ...
    """Translate Rust source into a policy-agnostic LanguageUnit."""
