"""Dart translator for DevCovenant LanguageUnit generation."""
...
TEST_TEMPLATES = ("{stem}_test.dart", "test_{stem}.dart")
SYMBOL_PATTERNS = (
    ("class", re.compile(r"\bclass\s+([A-Za-z_][A-Za-z0-9_]*)")),
    (
        "function",
        re.compile(
            r"\b(?:void|int|double|num|bool|String|Future(?:<[^>]+>)?)\s+"
            r"([A-Za-z_][A-Za-z0-9_]*)\s*\("
        ),
    ),
    (
        "variable",
        re.compile(r"\b(?:final|var|const)\s+([A-Za-z_][A-Za-z0-9_]*)"),
    ),
)
RISK_PATTERNS = (
    (
        re.compile(r"\bProcess\.(?:run|runSync|start)\s*\("),
        "Review external process execution.",
    ),
    (
        re.compile(r"\bPlatform\.environment\b"),
        "Review environment-variable access.",
    ),
    (re.compile(r"\bdynamic\b"), "Review dynamic typing usage."),
)
...
def _has_nearby_comment(line_number: int, lines: list[str]) -> bool:
    """Return True when a nearby Dart comment marker is present."""
...
def translate(
    *, path: Path, source: str, declaration: TranslatorDeclaration, **_: Any
    ...
    """Translate Dart source into a policy-agnostic LanguageUnit."""
