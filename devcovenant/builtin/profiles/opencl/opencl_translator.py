"""OpenCL translator for DevCovenant LanguageUnit generation."""
...
TEST_TEMPLATES = ("{stem}_test.cl", "tests/{stem}.cl")
SYMBOL_PATTERNS = (
    (
        "kernel",
        re.compile(r"\b__kernel\s+void\s+([A-Za-z_][A-Za-z0-9_]*)\s*\("),
    ),
    (
        "function",
        re.compile(
...
RISK_PATTERNS = (
    (
        ...
        "Review kernel-side printf usage.",
    ),
    (
        re.compile(r"\b__asm__?\s*\("),
        "Avoid inline assembly in portable OpenCL kernels.",
    ),
)
...
def _has_nearby_comment(line_number: int, lines: list[str]) -> bool:
    """Return True when a nearby OpenCL comment marker is present."""
...
def translate(
    *, path: Path, source: str, declaration: TranslatorDeclaration, **_: Any
    ...
    """Translate OpenCL source into a policy-agnostic LanguageUnit."""
